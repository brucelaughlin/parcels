...
polygon_seed_i_j_file = "/home/blaughli/parcels/general_code/polygons_and_seeding/z_output/seeding_ij_WCR30_withPeteAdditions_romsGridCellCentroidsOnly.p"
...
Y,X = (np.array(range(0,np.shape(mask_rho)[0])),np.array(range(0,np.shape(mask_rho)[1])))



fig, ax = plt.subplots()
#m = ax.pcolormesh(lon_rho,lat_rho,mask_rho,shading="nearest")
m = ax.pcolormesh(X,Y,mask_rho)
...
#ax.scatter(points_ij[:,0], points_ij[:,1], c='c', s=15)
ax.scatter(points_ij[:,1], points_ij[:,0], c='c', s=15)   # WHY REVERSED?
