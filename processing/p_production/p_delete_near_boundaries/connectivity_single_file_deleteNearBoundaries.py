...
grandparent_dir = "z_binned_settlement_counts"
...
settlement_count_parent_dir_local_name = f"binned_data_{version}"

cwd = os.getcwd()
parent_dir = os.path.join(str(cwd), grandparent_dir, settlement_count_parent_dir_local_name)
...
output_file_name = f"{input_year_str}{input_month_str}{input_day_str}_PLD_{pld_start_day:03d}_{pld_end_day:03d}"
...
for time_dex in range(pld_end_day_timestep):
    ...
    print(f"Time index: {time_dex + 1}/{pld_end_day_timestep}")
    ...
    if time_dex >= pld_start_day_timestep:
        for polygon_dex in range(num_polygons):
            ...
            #unsettled_particles_in_current_polygon_mask = np.logical_and(domain_inside_flags, np.logical_and(polygon_inside_flags, unsettled_particle_mask))
