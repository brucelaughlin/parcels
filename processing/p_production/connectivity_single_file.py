...
grandparent_dir = "z_binned_preconnectivty_data"
...
parser.add_argument("--pldstartday", type=int)
parser.add_argument("--pldendday", type=int)
args = parser.parse_args()

version = args.version
polygon_file = args.polygonfile
tracking_file = args.trackingfile
pld_start_day = args.pldstartday
pld_end_day = args.pldendday

'''
#---------------------------------------------------------------------
# TESTING with constants rather than input
#---------------------------------------------------------------------
version = "oldPolygonsNoPete_v1"
polygon_file = "/home/blaughli/parcels/general_code/polygons_and_seeding/z_input_files/bounding_boxes_lonlat_WCR30_singleCoastalCells.txt"
tracking_file = "/data04/cedwards/wcr30_parcels/wcr30_ERA1_v1/output_20251003/output_1999/z_output_19990103/wcr30_ERA_oneRelease_199913.zarr"
#tracking_file = "/data04/cedwards/wcr30_parcels/wcr30_ERA1_v1/output_20251003/output_1999/z_output_19990101/wcr30_ERA_oneRelease_199911.zarr"
pld_start_day = 1
pld_end_day = 5
#---------------------------------------------------------------------
'''

connectivity_parent_dir_local_name = f"binned_data_{version}"

cwd = os.getcwd()
parent_dir = os.path.join(str(cwd), grandparent_dir, connectivity_parent_dir_local_name)
...
output_file_name = f"PLD_{pld_start_day:03d}_{pld_end_day:03d}"
...
pld_start_day_timestep = int(pld_start_day / days_per_output_timestep)
pld_end_day_timestep = int(pld_end_day / days_per_output_timestep)
...
for polygon_dex in range(num_polygons):
    ...
    particles_inside_flags = path.contains_points(points_lon_lat_timestep_zero)
    particle_release_polygons[np.logical_and(particles_inside_flags,unprocessed_particle_mask)] = polygon_dex + 1
    unprocessed_particle_mask[particles_inside_flags] = False
...
for time_dex in range(pld_start_day_timestep, pld_end_day_timestep):
    
    #print('timestep within pld: {}/{}'.format(time_dex - pld_start_day_timestep +1, pld_end_day_timestep - pld_start_day_timestep))
    ...
    for polygon_dex in range(num_polygons):
        ...
        particles_inside_flags = path.contains_points(points_lon_lat_current_timestep)
        
        unsettled_particles_in_current_polygon_mask = np.logical_and(particles_inside_flags, unsettled_particle_mask)  
...
binned_connectivity_data = np.zeros((num_polygons,num_polygons))

for particle_num in range(num_particles):
    if particle_release_polygons[particle_num] == 0 or particle_settlement_polygons[particle_num] == 0:
        continue
    else:
        binned_connectivity_data[int(particle_release_polygons[particle_num])-1,int(particle_settlement_polygons[particle_num])-1] += 1   
...
d['binned_connectivity_data'] = binned_connectivity_data
